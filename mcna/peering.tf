...
resource "aviatrix_transit_gateway_peering" "aws_gcp" {
  ...
  depends_on            = [aviatrix_transit_gateway.aws_transit_gw, aviatrix_transit_gateway.gcp_transit_gw]
}

### Azure <--> GCP transit peering.
resource "aviatrix_transit_gateway_peering" "azure_gcp" {
  transit_gateway_name1 = var.azure_transit_gateway.name
  transit_gateway_name2 = var.gcp_transit_gateway.name
  depends_on            = [aviatrix_transit_gateway.azure_transit_gw, aviatrix_transit_gateway.gcp_transit_gw]
}
